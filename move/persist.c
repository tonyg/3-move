...
PUBLIC OBJ load(void *handle) {
  ...
  switch (t) {
	...
	case KIND_OBJECT: {
	  ...
	  obj->methods = (OVECTOR) load(handle);
	  obj->attributes = (OVECTOR) load(handle);
	  obj->parents = load(handle);
	  obj->owner = (OBJECT) load(handle);
	  obj->group = (VECTOR) load(handle);
	  obj->location = (OBJECT) load(handle);
	  obj->contents = (VECTOR) load(handle);
    ...
    default:
      fprintf(stderr, "unknown type-char (%c) in load()!\n", t);
      exit(2);
  }
